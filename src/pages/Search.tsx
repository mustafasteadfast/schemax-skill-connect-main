...
import { getFreelancersWithSkills } from "@/lib/mock-data";
import { 
...
export default function Search() {
  ...
  const searchFreelancers = async () => {
    ...
    try {
      ...
      let freelancers = getFreelancersWithSkills();
      ...
      if (searchQuery) {
        ...
        freelancers = freelancers.filter(f =>
          ...
          f.skills?.some(skill => skill.name.toLowerCase().includes(query))
        );
      }
      ...
      if (minRate[0] > 0) {
        freelancers = freelancers.filter(f => (f.hourly_rate || 0) >= minRate[0]);
      }

      if (maxRate[0] < 200) {
        freelancers = freelancers.filter(f => (f.hourly_rate || 0) <= maxRate[0]);
      }

      // Mock category and skills filtering from URL
      ...
      if (skills) {
        ...
        freelancers = freelancers.filter(f =>
          f.skills?.some(skill => skillList.includes(skill.name))
        );
      }

      if (category) {
        // Mock category filtering - just filter by having services
        freelancers = freelancers.filter(f => f.services && f.services.length > 0);
      }

      setFreelancers(freelancers.slice(0, 20)); // Limit to 20 results
    } catch (error: any) {
      toast({
        title: "Search failed",
